...
function buildRuntimeContext(parsed: ParsedArgv): RuntimeContext {
  ...
  if (
    !executionMode &&
    !normalizeNonEmptyString(process.env.AGENTSPACE_CLI_EXECUTION_MODE) &&
    isSqliteRepoUrl(repoUrl)
  ) {
    process.env.AGENTSPACE_CLI_EXECUTION_MODE = 'tooling'
  }
...
void main().catch((error) => {
  ...
  const logLevel = String(process.env.LOG_LEVEL ?? '').trim().toLowerCase()
  const output = logLevel === 'debug' || logLevel === 'trace'
    ? inspect(error, { depth: 8, colors: false })
    : message
